...
/* 鈹€鈹€ Main watch component 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
function WatchContent() {
  ...
  // 鈹€鈹€ Transcript / translation localStorage cache 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  const CACHE_TTL = 30 * 24 * 60 * 60 * 1000;        // 30 days 鈥?transcript text (stable)
  const TRANSLATION_CACHE_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days 鈥?translations may be updated
  ...
  const writeTranscriptCache = (slug: string, transcript: TranscriptItem[]) => {
    ...
    } catch { /* quota exceeded 鈥?silently skip */ }
  };
  ...
  // playbackRate = 1 is mandatory 鈥?at higher rates the audio is
  // time-compressed and ASR models trained on normal-speed speech will fail.
  const captureAudioFromHiddenVideo = (
    onStatus: (s: string) => void
  ): Promise<Float32Array> =>
    new Promise((resolve, reject) => {
      if (!data?.videoUrl) { reject(new Error('瑙嗛婧愪笉鍙敤')); return; }

      // Hidden video 鈥?must be in the DOM for autoplay to work
      ...
      cap.muted = true;   // muted 鈫?autoplay allowed without prior gesture
      ...
      const startCapture = () => {
        ...
        try {
          // muted=true does NOT silence MediaElementAudioSourceNode 鈥?          // the Web Audio API intercepts decoded PCM before the mute flag applies.
          source = audioCtx.createMediaElementSource(cap);
        } catch {
          audioCtx.close();
          teardown();
          reject(new Error('鏃犳硶缁戝畾瑙嗛闊抽婧愶紝璇峰埛鏂伴〉闈㈠悗閲嶈瘯'));
          return;
        }
        ...
        onStatus(`姝ｅ湪鎻愬彇闊抽 0%${durSec > 0 ? `锛堢害闇€ ${Math.ceil(durSec / 60)} 鍒嗛挓锛塦 : ''}...`);
        ...
        const onTimeUpdate = () => {
          ...
          if (d > 0 && isFinite(d)) {
            ...
            onStatus(`姝ｅ湪鎻愬彇闊抽 ${pct}%${remMin > 0 ? `锛堣繕闇€绾?${remMin} 鍒嗛挓锛塦 : ''}...`);
          }
        };
        ...
        cap.addEventListener('error', () => finish(new Error('瑙嗛鎹曡幏鍑洪敊')), { once: true });

        // playbackRate MUST stay at 1脳.
        // Higher rates time-compress the audio 鈥?ASR models expect normal-speed speech.
        cap.playbackRate = 1;
      ...
      cap.addEventListener('error', () => {
        teardown();
        reject(new Error('瑙嗛鍔犺浇澶辫触锛屾棤娉曟彁鍙栭煶棰?));
      }, { once: true });
    });
  ...
  const handleTranscribe = async () => {
    ...
    try {
      ...
      // 1. Server-side HLS audio extraction 鈥?parallel segment fetch + TS demux on server
      ...
      if (!float32Data && data.isHls && data.hlsUrl) {
        try {
          setTranscribeStatus('姝ｅ湪鎻愬彇闊抽 (鏈嶅姟绔В鏋愪腑)...');
      ...
      for (const url of sources) {
        try {
          ...
          if (res.ok) {
            ...
            const ct = res.headers.get('content-type') || '';
            if (ct.includes('text/html')) {
              lastError = '鏀跺埌 HTML 鑰岄潪闊抽 (鍙兘闇€瑕佺櫥褰?';
              continue;
            }
            const arrayBuffer = await res.arrayBuffer();
            const audioCtx = new (window.AudioContext || (window as any).webkitAudioContext)({ sampleRate: 16000 });
      ...
      if (!float32Data) {
        ...
        setTranscribeStatus('姝ｅ湪鍔犺浇瑙嗛浠ユ彁鍙栭煶棰?..');
      ...
      setTranscribeStatus('鍒濆鍖?AI 寮曟搸...');
      workerRef.current.postMessage({ type: 'transcribe', audio: float32Data });
    } catch (err: any) {
      console.error('[ASR] Fatal error:', err);
      alert('闊抽澶勭悊澶辫触: ' + err.message);
      setIsTranscribing(false);
    }
  };

  // AbortController for in-flight subtitle fetch 鈥?cancelled on rapid language switches
  ...
  const handleAiTranslate = async () => {
    ...
    if (user.credits < AI_TRANSLATE_COST) {
      alert(lang === "en"
        ...
        : `绉垎涓嶈冻锛岄渶瑕?${AI_TRANSLATE_COST} 绉垎`);
      return;
    }
  ...
  /* 鈹€鈹€ RAF sync (throttled: only update when time changes by >50ms) 鈹€鈹€ */
  ...
  /* 鈹€鈹€ Hotkeys 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  useEffect(() => {
  ...
  /* 鈹€鈹€ Initial fetch 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  ...
  /* 鈹€鈹€ HLS setup 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  useEffect(() => {
    ...
    // 鎭㈠涔嬪墠鐨勬挱鏀炬柇鐐?    const startProgress = searchParams.get("t");
    ...
    return () => {
      if (hls) hls.destroy();
      // 鍗歌浇鏃朵笂鎶ヨ繘搴?鈥?use ref so this doesn't re-run when user credits update
  ...
  /* 鈹€鈹€ Active index 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  ...
  /* 鈹€鈹€ Virtual transcript list 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  ...
  // AbortController for in-flight word lookup 鈥?cancelled when user clicks a different word
  ...
  const handleDeepAnalyze = async (item: TranscriptItem, preloaded?: any) => {
    ...
    // Already cached 鈥?just open the panel
  ...
  /* 鈹€鈹€ Loading screen 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  if (loading) return (
  ...
  /* 鈹€鈹€ Print styles 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      {/* 鈹€鈹€ Header 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
      <header className="h-14 flex items-center gap-2 px-4 border-b shrink-0 print-hidden"
        ...
        {/* Knowledge Hub 鈥?primary action, always visible */}
              ...
              <div className="absolute top-full right-0 mt-2 w-64 rounded-2xl shadow-2xl z-[110] py-2 overflow-hidden"
                ...
                <div className="px-3 pb-1">
                  ...
                  <button onClick={toggleTheme}
                    ...
                    {theme === "dark" ? (lang === "en" ? "Light Mode" : "娴呰壊妯″紡") : (lang === "en" ? "Dark Mode" : "娣辫壊妯″紡")}
                  </button>

                  {/* App language */}
                  <div className="px-2 py-1.5">
                    <p className="text-xs mb-1.5 flex items-center gap-1.5" style={{ color: "var(--text-2)" }}>
                      <Globe size={13} style={{ color: "var(--accent)" }} />{lang === "en" ? "Language" : "鐣岄潰璇█"}
                    </p>
      ...
      {/* 鈹€鈹€ History Modal 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
      {showHistory && (
        <HistoryModal
      ...
      {/* 鈹€鈹€ Main layout 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
      <main className="flex-1 flex flex-col md:flex-row overflow-hidden print-hidden">

        {/* Notebook overlay 鈥?full-height right drawer with backdrop */}
        {showVocab && (
        ...
        {/* 鈹€鈹€ Video player 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
        ...
        {/* 鈹€鈹€ Transcript panel 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
            ...
            {data?.isTranslationMissing && subtitleLang !== "en" && (
              <div className="ml-auto flex items-center gap-2">
                <span className="hidden xl:inline text-[10px] whitespace-nowrap" style={{ color: "var(--text-3)" }}>
                  {lang === "en" ? "No official sub" : "鏆傛棤瀹樻柟瀛楀箷"}
                </span>
                {user ? (
                  <button onClick={handleAiTranslate} disabled={isAiTranslating}
                    ...
                    {isAiTranslating
                      ...
                      : <><Sparkles size={10} /> {t.aiTranslate} 路 {AI_TRANSLATE_COST}</>}
                  </button>
                ) : (
        ...
        {/* 鈹€鈹€ AI Analysis panel (3rd column) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
      ...
      {/* 鈹€鈹€ Word modal 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ */}
      {activeWord && (
        <WordLookupModal
