...
// Cache for 1 hour — TED updates featured talks a few times a day at most
...
function pickThumbnail(primaryImageSet: any[]): string {
  ...
  // Prefer 16x9 aspect ratio; otherwise take the first entry
...
export async function GET() {
  ...
  try {
    ...
    if (!raw) return NextResponse.json({ talks: [] });
    ...
    const talks: FeaturedTalk[] = rawTalks
      ...
      .map(t => ({
        title: t.title,
        presenter: t.presenterDisplayName ?? "",
