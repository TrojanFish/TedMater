...
export default function AIAnalysisPanel({ analysis, loading, sentence, savedSentences, onClose, onSave }: Props) {
  const { t } = useApp();
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-6 p-12">
            <div className="w-16 h-16 rounded-2xl bg-accent/10 border-2 border-accent/20 flex items-center justify-center animate-pulse">
              <Sparkles size={28} className="text-accent" strokeWidth={2.5} />
            </div>
            <div className="text-center space-y-2">
              <p className="text-sm font-black uppercase tracking-widest text-foreground">Analyzing...</p>
              <p className="text-xs font-bold text-muted-foreground">AI is breaking down the sentence</p>
            </div>
            <div className="w-full max-w-xs space-y-3">
              {[80, 60, 90, 50].map((w, i) => (
                <div key={i} className={`h-3 bg-muted rounded-full animate-pulse`} style={{ width: `${w}%`, animationDelay: `${i * 150}ms` }} />
              ))}
            </div>
          </div>
        ) : (
